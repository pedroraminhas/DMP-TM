...
# endif /* WAIT_YIELD */
  ...
  int_stm_prepare(tx);
						if(!local_exec_mode){
							memset(pages_read,0,NUM_PAGES); 
                            memset(pages_written,0,NUM_PAGES); 
                            memset(transition_count_pages_read,0,NUM_PAGES); 
						}

	int loop_counter;
    int pageID;
                     if(counter_pages_written>0) {\
                      	for(loop_counter=0; loop_counter < counter_pages_written; loop_counter++){ \
                          	pageID=pages_written_heap[loop_counter]; \
                            writer_count[threadID][pageID]=0; \
                          /*__sync_sub_and_fetch(&metadata_array[pageID].writer_count,1); */\
                          /*printf("commit THREAD = %d PAGE= %d WRITER COUNT DECREMENTED TO= %d\n",SPECIAL_THREAD_ID(),pageID,metadata_array[pageID].writer_count); */\
                      	}   \
