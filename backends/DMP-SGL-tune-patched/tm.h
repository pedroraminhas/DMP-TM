...
#define TM_STARTUP(numThread,useless)     \
                        ...
                        for(loop_counter_startup=0;loop_counter_startup<5;++loop_counter_startup) \
                            sched_mode[loop_counter_startup].counter = 0; \
                        ...
                        if (status == _HTM_TBEGIN_STARTED) { \
                            ...
                            break; \
                            		...
                            		if (__TM_is_footprint_exceeded(&TM_buff)) { \
                                        ...
                                        if(!bailout){ \
    										sched_capacity[num_tx].counter++; \
                            				long temp = sched_htm_commits[num_tx].counter+sched_capacity[num_tx].counter; \
                            				if(temp>100 && (sched_capacity[num_tx].counter * 100/ temp) >= sched_threshold){ \
                              					sched_mode[num_tx].counter=1; \
                                            	STM_BEGIN(b,ro); \
                              					break; \
                                			} \
                                        } \
...
# define TM_BEGIN(b,ro)           TM_BEGIN_EXT(b,0)
# define TM_BEGIN_EXT(b,ro) {  \
                update_transaction=!ro; \
                num_tx = b; \
                if(sched_mode[num_tx].counter == 2){ \
                    VSTM_BEGIN(b,ro); \
                } \
                else if(sched_mode[num_tx].counter==1){ \
                    STM_BEGIN(b,ro); \
                } else{ \
                    HTM_BEGIN(b,ro); \
                } \
    ...
    if(local_exec_mode == 1) { \
        ...
        if(local_transition_count != global_transition_count){ \
            for(loop_counter=0; loop_counter< counter_pages_read; loop_counter++){ \
                ...
                if(metadata_array[pageID].transition_count != transition_count_pages_read[pageID]){ \
                    SLOW_PATH_RESTART(); \
                    transition_count_pages_read[pageID] = 0; \
                }    \   
            }   \   
        }   \   
        } \ 
		...
		statistics_array[SPECIAL_THREAD_ID()].stm_aborts--; \
		statistics_array[SPECIAL_THREAD_ID()].stm_commits++; \
	}else if (local_exec_mode==0) { \
        __TM_end(); \
        sched_htm_commits[num_tx].counter++; \
...
#define VALIDATE_READSET(){ \
    ...
    if(local_transition_count!=temp_gtc){ \
        for(loop_counter_read=0;loop_counter_read<counter_pages_read;loop_counter_read++){ \
            ...
            if(((pages_read & msk) == msk) && (transition_count_pages_read[pageID_aux] != metadata_array[pageID_aux].transition_count)){ \
                    SLOW_PATH_RESTART(); \
            } \
        } \
...
# define SLOW_PATH_SHARED_READ(var) ({    long value_read=0; \
    ...
    if(!((pages_read & msk) == msk)){ \
        ...
        transition_count_pages_read[pageID] = metadata_array[pageID].transition_count; \
        counter_pages_read++; \
    } \
...
# define BAILOUT_RESTART() SLOW_PATH_RESTART()
# define BAILOUT_SHARED_READ(var)          stm_load((volatile stm_word_t *)(void *)&(var)); 
# define BAILOUT_SHARED_WRITE(var, val)     stm_store((volatile stm_word_t *)(void *)&(var), (stm_word_t)(val)); 
