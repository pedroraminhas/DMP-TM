...
namespace sb7 {
	...
	void *tm_read_word(void *addr, int mode);

	void tm_write_word(void *addr, void *val, int mode);
...
inline void* sb7::tm_read_word(void* addr, int mode) {
	if (mode == 0){
		return (void*)(FAST_PATH_SHARED_READ(*((intptr_t*)addr)));
	}
	else{
		return (void*)(SLOW_PATH_SHARED_READ_P(*((intptr_t*)addr)));
	}
}

inline void sb7::tm_write_word(void* addr, void* val, int mode) {
	if (mode == 0)
		FAST_PATH_SHARED_WRITE(*((intptr_t*)addr), (intptr_t)val);
	else
		SLOW_PATH_SHARED_WRITE_P(*((intptr_t*)addr), (intptr_t)val);
}
