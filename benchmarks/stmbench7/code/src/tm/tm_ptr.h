...
namespace sb7 {
	...
	template <typename T> void tx_delete(
		sh_ptr<T>& delete_on_tx_commit, int mode);
	...
	class sh_ptr {
		public:
			...
			friend void tx_delete<>(sh_ptr<T>&, int);
    ...
    class rd_ptr {
		...
		public:
			...
			rd_ptr(const int must_be_null, int mode);
			rd_ptr(const rd_ptr<T>& copy, int mode);
			explicit rd_ptr(const sh_ptr<T>& to_open, int mode);
    ...
    class wr_ptr {
		...
		public:
			...
			wr_ptr(const int must_be_null, int mode);
			wr_ptr(const wr_ptr<T>&, int mode);
			explicit wr_ptr(const sh_ptr<T>&, int mode);
...
namespace sb7 {
	...
	inline const T *rd_ptr<T>::open_shared(const sh_ptr<T>& shared) {
		return (const T *)tm_read_word((void *)&(shared.obj->obj), mode);
	}

	template <typename T>
	inline rd_ptr<T>::rd_ptr(const int must_be_null, int mode) : obj(NULL) {
		mode = mode;
		assert(must_be_null == 0);
	}

	template <typename T>
	inline rd_ptr<T>::rd_ptr(const rd_ptr<T>& ptr, int mode) : obj(ptr.obj) { mode = mode; }

	template <typename T>
	inline rd_ptr<T>::rd_ptr(const sh_ptr<T>& shared, int mode) {
		mode = mode;
...
namespace sb7 {
	...
	template <typename T>
	inline wr_ptr<T>::wr_ptr(const int must_be_null, int mode) : obj(NULL) {
		mode = mode;
		assert(must_be_null == 0);
	}

	template <typename T>
	inline wr_ptr<T>::wr_ptr(const wr_ptr<T>& ptr, int mode) : obj(ptr.obj) { mode = mode; }

	template <typename T>
	inline wr_ptr<T>::wr_ptr(const sh_ptr<T>& ptr, int mode) {
		mode = mode;
		obj = open_shared(ptr);
	}
