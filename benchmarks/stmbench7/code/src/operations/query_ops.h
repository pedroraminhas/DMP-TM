...
namespace sb7 {
	...
	class Query5 : public Operation {
		...
		protected:
			int checkBaseAssembly(sh_ptr<BaseAssembly> bassm, int mode) const;
	};
	...
	class Query6 : public Query5 {
		...
		protected:
			int checkAssembly(sh_ptr<Assembly> assembly, int mode) const;
			int checkComplexAssembly(sh_ptr<ComplexAssembly> assembly, int mode) const;
	};
