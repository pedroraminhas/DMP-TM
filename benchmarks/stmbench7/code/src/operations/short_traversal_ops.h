...
namespace sb7 {
	...
	class ShortTraversal1 : public Operation {
		...
		protected:
			int traverse(sh_ptr<Assembly> assm, int mode) const;
			int traverse(sh_ptr<ComplexAssembly> cassm, int mode) const;
			int traverse(sh_ptr<BaseAssembly> bassm, int mode) const;
			virtual int traverse(sh_ptr<CompositePart> cpart, int mode) const;
			virtual int traverse(sh_ptr<AtomicPart> apart, int mode) const;
	};
	...
	class ShortTraversal2 : public ShortTraversal1 {
		...
		protected:
			...
			virtual int traverse(sh_ptr<Document> documentation, int mode) const;
	};
