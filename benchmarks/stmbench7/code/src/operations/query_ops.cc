...
int sb7::Query1::run(int mode) const {
	int count = 0;
	...
	for(int i = 0; i < QUERY1_ITER; i++) {
		...
		if(query.found) {
			performOperationOnAtomicPart(query.val, mode);
			count++;
		}
	}
...
int sb7::Query2::run(int mode) const {
	...
	while(iter.has_next()) {
		...
		while(apartIter.has_next()) {
			...
			performOperationOnAtomicPart(apart, mode);
			count++;
		}
	}
...
int sb7::Query4::run(int mode) const {
	int ret = 0;
	...
	for(int i = 0;i < QUERY4_ITER;i++) {
		...
		rd_ptr<Map<string, sh_ptr<Document> > > rd_docInd(
			dataHolder->getDocumentTitleIndex(), mode);
		Map<string, sh_ptr<Document> >::Query query;
		query.key = title;
		rd_docInd->get(query);

		if(query.found) {
			rd_ptr<Document> rd_doc(query.val, mode);
			rd_ptr<CompositePart> rd_cpart(rd_doc->getCompositePart(),  mode);
			rd_ptr<Bag<sh_ptr<BaseAssembly> > > rd_usedIn(
				rd_cpart->getUsedIn(), mode);
			BagIterator<sh_ptr<BaseAssembly> > iter = rd_usedIn->getIter();

			while(iter.has_next()) {
				rd_ptr<BaseAssembly> rd_bassm(iter.next(), mode);
...
int sb7::Query5::run(int mode) const {
	int ret = 0;
	...
	while(iter.has_next()) {
		ret += checkBaseAssembly(iter.next(), mode);
	} 

	return ret;
}

int sb7::Query5::checkBaseAssembly(sh_ptr<BaseAssembly> bassm, int mode) const {
	...
	while(iter.has_next()) {
		rd_ptr<CompositePart> rd_cpart(iter.next(), mode);
...
int sb7::Query6::run(int mode) const {
	rd_ptr<Module> rd_module(dataHolder->getModule(), mode);
	return checkComplexAssembly(rd_module->getDesignRoot(), mode);
}

int sb7::Query6::checkAssembly(sh_ptr<Assembly> assembly, int mode) const {
	rd_ptr<Assembly> rd_assm(assembly, mode);

	if(rd_assm->getType() == assembly_type_complex) {
		return checkComplexAssembly(assembly, mode);
	} else {
		return checkBaseAssembly(assembly, mode);
	}
}

int sb7::Query6::checkComplexAssembly(sh_ptr<ComplexAssembly> assembly, int mode) const {
	int ret = 0;

	rd_ptr<ComplexAssembly> rd_assembly(assembly, mode);
	rd_ptr<Set<sh_ptr<Assembly> > > rd_subAssmSet(
		rd_assembly->getSubAssemblies(), mode);
	SetIterator<sh_ptr<Assembly> > iter = rd_subAssmSet->getIter();

	while(iter.has_next()) {
		ret += checkAssembly(iter.next(), mode);
	}
...
int sb7::Query7::run(int mode) const {
	int ret = 0;
	...
	while(iter.has_next()) {
		rd_ptr<AtomicPart> rd_apart(iter.next(), mode);
		rd_apart->nullOperation();
		ret++;
	}
