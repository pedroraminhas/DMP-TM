...
namespace sb7 {
	...
	class Traversal1 : public Operation {
		...
		public:
			...
			virtual int run(int mode) const;

		protected:
			int traverse(sh_ptr<ComplexAssembly> complexAssembly, int mode) const;
			int traverse(sh_ptr<BaseAssembly> baseAssembly, int mode) const;
			virtual int traverse(sh_ptr<CompositePart> component, int mode) const;
			virtual int traverse(sh_ptr<AtomicPart> part,
				Set<sh_ptr<AtomicPart> > &setOfVisitedParts, int mode) const;
			virtual int performOperationOnAtomicPart(sh_ptr<AtomicPart> apart,
				Set<sh_ptr<AtomicPart> > &setOfVisitedPartIds, int mode) const;
	};
	...
	class Traversal3a : public Traversal1 {
		...
		protected:
			...
			void updateBuildDate(sh_ptr<AtomicPart> apart, int mode) const;
	};
	...
	class Traversal4 : public Traversal1 {
		...
		protected:
			virtual int traverse(sh_ptr<CompositePart> component, int mode) const;
			virtual int traverse(sh_ptr<Document> doc, int mode) const;
			virtual int traverse(sh_ptr<AtomicPart> part,
	...
	class Traversal7 : public Operation {
		...
		protected:
			int traverse(sh_ptr<CompositePart> cpart, int mode) const;
	...
	class Traversal8 : public Operation {
		...
		protected:
			virtual int traverse(sh_ptr<Manual> manual, int mode) const;
	};
