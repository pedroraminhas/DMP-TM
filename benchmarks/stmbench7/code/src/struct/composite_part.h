...
namespace sb7 {
	...
	class CompositePart : public DesignObj {
		...
		public:
			...
			void addAssembly(sh_ptr<BaseAssembly> assembly, int mode) const {
				wr_ptr<base_assembly_bag> wr_usedIn(m_usedIn, mode);
				wr_usedIn->add(assembly);
			}

			void removeAssembly(sh_ptr<BaseAssembly> assembly, int mode) const {
				wr_ptr<base_assembly_bag> wr_usedIn(m_usedIn, mode);
				wr_usedIn->remove(assembly);
			}

			bool addPart(sh_ptr<AtomicPart> part, int mode);
			...
			void freeMemory(int mode) {
				tx_delete(m_usedIn, mode);
				tx_delete(m_parts, mode);
			}
