...
namespace sb7 {
	...
	class ComplexAssembly : public Assembly {
		...
		public:
			...
			bool addSubAssembly(sh_ptr<Assembly> assembly, int mode) const;

			bool removeSubAssembly(sh_ptr<Assembly> assembly, int mode) const;
			...
			void freeMemory(int mode) {
				tx_delete(m_subAssemblies, mode);
			}
	...
	class BaseAssembly : public Assembly {
		...
		public:
			...
			void addComponent(sh_ptr<CompositePart> cpart, int mode) const;

			bool removeComponent(sh_ptr<CompositePart> cpart, int mode) const;
