...
namespace sb7 {
	...
	class AtomicPart : public DesignObj {
		...
		public:
			...
			void addConnectionFromOtherPart(sh_ptr<Connection> conn, int mode) const {
				wr_ptr<connection_set> wr_from(m_from, mode);
				wr_from->add(conn);
			}

			void removeConnectionFromOtherPart(sh_ptr<Connection> conn, int mode) const {
				wr_ptr<connection_set> wr_from(m_from, mode);
				wr_from->remove(conn);
			}

			void removeConnectionToOtherPart(sh_ptr<Connection> conn, int mode) const {
			...
			int getNumConnections(int mode) const {
				rd_ptr<connection_set> rd_to(m_to, mode);
				return rd_to->size();
			}
