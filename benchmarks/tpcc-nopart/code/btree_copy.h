...
public:
    ...
    __attribute__((transaction_safe)) void slow_insert(TM_ARGDECL KEY key, VALUE value) {
      ...
      InsertionResult result = { KEY(), 0, 0 };
      bool was_split;
      unsigned long d = STM_SHARED_READ(depth);
      if( d == 0 ) {
        // The root is a leaf node
        void* temp_node = STM_SHARED_READ_P(root);
      ...
      if( was_split ) {
        ...
        rootProxy->children[0]= result.left;
        rootProxy->children[1]= result.right;
        STM_SHARED_WRITE(depth,++d);
        STM_SHARED_WRITE_P(root,rootProxy);
      }
    }
...
__attribute__((transaction_safe))  bool slow_find(TM_ARGDECL const KEY& key, VALUE* value= 0) const {
  const InnerNode* inner;
  register const void* node= STM_SHARED_READ_P(root);
  register unsigned long d= STM_SHARED_READ(depth);
  register unsigned  index;
  while( d-- != 0 ) {
    inner= reinterpret_cast<const InnerNode*>(node);
    unsigned long num_keys_1 = STM_SHARED_READ(inner->num_keys);
    index= slow_inner_position_for(TM_ARG key, inner->keys, num_keys_1);
    node= STM_SHARED_READ_P(inner->children[index]);
  }
  const LeafNode* leaf= reinterpret_cast<const LeafNode*>(node);
  unsigned long num_keys = STM_SHARED_READ(leaf->num_keys);
  index= slow_leaf_position_for(TM_ARG key, leaf->keys, num_keys);
  if( index < num_keys ) {
    intptr_t temp_key = STM_SHARED_READ(leaf->keys[index]);
    if(temp_key == key){
    VALUE temp_value = (VALUE)STM_SHARED_READ_P(leaf->values[index]);
    if( value != 0 ) {
      *value = temp_value;
    }
    if (temp_value)
...
__attribute__((transaction_safe)) bool slow_del(TM_ARGDECL const KEY& key) {
  InnerNode* inner;
  register void* node= STM_SHARED_READ_P(root);
  register unsigned long d= STM_SHARED_READ(depth);
  register unsigned  index;
  while( d-- != 0 ) {
    inner= reinterpret_cast<InnerNode*>(node);
  ...
  intptr_t temp_key = STM_SHARED_READ(leaf->keys[index]);
  if( temp_key == key ) {
    STM_SHARED_WRITE_P(leaf->values[index], 0);
    return true;
  } else {
    return false;
  }
...
__attribute__((transaction_safe)) bool fast_findLastLessThan(TM_ARGDECL const KEY& key, VALUE* value = 0, KEY* out_key = 0) const {
    ...
    __attribute__((transaction_safe)) bool slow_findLastLessThan(TM_ARGDECL const KEY& key, VALUE* value = 0, KEY* out_key = 0) const {
        const void* node = STM_SHARED_READ_P(root);
        unsigned long d = STM_SHARED_READ(depth);
        while( d-- != 0 ) {
          const InnerNode* inner = reinterpret_cast<const InnerNode*>(node);
    	    unsigned long num_keys_1 = STM_SHARED_READ(inner->num_keys);
          ...
          // We need to rewind in the case where they are equal
          if (pos > 0) {
            intptr_t temp_key = STM_SHARED_READ(inner->keys[pos-1]);
            if (key == temp_key)
    	       pos -= 1;
           }
           node = STM_SHARED_READ_P(inner->children[pos]);
        }
        const LeafNode* leaf= reinterpret_cast<const LeafNode*>(node);
        unsigned long num_keys = STM_SHARED_READ(leaf->num_keys);
        ...
        if (pos <= num_keys) {
            pos -= 1;
            if (pos < num_keys) {
              intptr_t temp_key_2 = STM_SHARED_READ(leaf->keys[pos]);
              if (key == temp_key_2)
                pos -= 1;
            }
            if (pos < num_keys) {
              VALUE temp_value = (VALUE)STM_SHARED_READ_P(leaf->values[pos]);
              if (temp_value) {
                if (value != NULL) {
                  *value = temp_value;
                }
                if (out_key != NULL) {
    		          intptr_t temp_key_3 = STM_SHARED_READ(leaf->keys[pos]);
    		          *out_key = temp_key_3;
                }
                return true;
              } else {
                ...
                // this makes it work. We need this for TPC-C undo. The solution is to use a
                // more complete b-tree implementation.
                intptr_t temp_key_4 = STM_SHARED_READ(leaf->keys[pos]);
...
private:
	  ...
	  //                unsigned char _pad[LEAF_NODE_PADDING];
        ...
        static unsigned slow_leaf_position_for(TM_ARGDECL const KEY& key, const KEY* keys,
                ...
                // Simple linear search. Faster for small values of N or M
                unsigned k= 0;
                intptr_t temp_key = STM_SHARED_READ(keys[k]);
                while((k < num_keys) && (temp_key<key)) {
                        ++k;
                        temp_key = STM_SHARED_READ(keys[k]);
                }
                return k;
                /*
                // Binary search. It is faster when N or M is > 100,
        ...
        __attribute__((transaction_safe)) bool slow_leaf_insert(TM_ARGDECL LeafNode* node, KEY& key,VALUE& value, InsertionResult* result) {
          bool was_split= false;
          // Simple linear search
		      unsigned long num_keys = STM_SHARED_READ(node->num_keys);
          ...
          if( num_keys == M ) {
            ...
            new_sibling->num_keys= num_keys -treshold;
            for(unsigned j=0; j < new_sibling->num_keys; ++j) {
              intptr_t temp_key = STM_SHARED_READ(node->keys[treshold+j]);
				      new_sibling->keys[j] = temp_key;
				      VALUE temp_value = (VALUE)STM_SHARED_READ_P(node->values[treshold+j]);
				      new_sibling->values[j] = temp_value;
            }
            STM_SHARED_WRITE(node->num_keys, treshold);
            if( i < treshold ) {
              // Inserted element goes to left sibling
            ...
            // Notify the parent about the split
            was_split= true;
			      result->key = STM_SHARED_READ(new_sibling->keys[0]);
            result->left= node;
            result->right= new_sibling;
          } else {
            // The node was not full
        ...
        __attribute__((transaction_safe))static void slow_leaf_insert_nonfull(TM_ARGDECL LeafNode* node, KEY& key, VALUE& value,unsigned index) {
          intptr_t temp_key = STM_SHARED_READ(node->keys[index]);
          if( temp_key == key ) {
            // We are inserting a duplicate value.
            // Simply overwrite the old one
			      VALUE temp_value = (VALUE)STM_SHARED_READ_P(value);
			      STM_SHARED_WRITE_P(node->values[index],temp_value);
          } else {
			      unsigned long num_keys = STM_SHARED_READ(node->num_keys);
            // The key we are inserting is unique
            for(unsigned i=num_keys; i > index; --i) {
              intptr_t temp_key_2 = STM_SHARED_READ(node->keys[i-1]);
				      STM_SHARED_WRITE(node->keys[i],temp_key_2);
				      VALUE temp_value_2 = (VALUE)STM_SHARED_READ_P(node->values[i-1]);
				      STM_SHARED_WRITE_P(node->values[i],temp_value_2);
            }
            STM_SHARED_WRITE(node->num_keys,++num_keys);
			      //node->num_keys++;
			      STM_SHARED_WRITE(node->keys[index],key);
			      VALUE temp_value_3 = (VALUE)STM_SHARED_READ_P(value);
			      STM_SHARED_WRITE_P(node->values[index],temp_value_3);
          }
        }
        ...
        __attribute__((transaction_safe)) bool slow_inner_insert(TM_ARGDECL InnerNode* node, unsigned long current_depth, KEY& key,VALUE& value, InsertionResult* result) {
          ...
          if( num_keys == N ) {
            ...
            new_sibling->num_keys= num_keys -treshold;
            for(unsigned i=0; i < new_sibling->num_keys; ++i) {
              intptr_t temp_key = STM_SHARED_READ(node->keys[treshold+i]);
              new_sibling->keys[i] = temp_key;
              new_sibling->children[i]= STM_SHARED_READ_P(node->children[treshold+i]);
            }
            new_sibling->children[new_sibling->num_keys]= STM_SHARED_READ_P(node->children[num_keys]);
			      STM_SHARED_WRITE(node->num_keys,treshold-1);
            // Set up the return variable
            was_split= true;
            intptr_t temp_key_2 = STM_SHARED_READ(node->keys[treshold-1]);
            result->key = temp_key_2;
            result->left= STM_SHARED_READ_P(node);
            result->right= new_sibling;
            // Now insert in the appropriate sibling
            if( key < result->key ) {
      ...
      __attribute__((transaction_safe)) void slow_inner_insert_nonfull(TM_ARGDECL InnerNode* node, unsigned long current_depth, KEY& key,VALUE& value) {
        ...
        if( current_depth-1 == 0 ) {
          // The children are leaf nodes
          void* temp_node = STM_SHARED_READ_P(node->children[index]);
          was_split= slow_leaf_insert(TM_ARG reinterpret_cast<LeafNode*>(temp_node), key, value, &result);
        } else {
          // The children are inner nodes
          void* temp_node = STM_SHARED_READ_P(node->children[index]);
        ...
        if( was_split ) {
          if( index == num_keys ) {
          // Insertion at the rightmost key
          STM_SHARED_WRITE(node->keys[index],result.key);
          STM_SHARED_WRITE_P(node->children[index], result.left);
          STM_SHARED_WRITE_P(node->children[index+1], result.right);
          STM_SHARED_WRITE(node->num_keys,++num_keys);
        } else {
          // Insertion not at the rightmost key
          void* temp_node_2 = STM_SHARED_READ_P(node->children[num_keys]);
          STM_SHARED_WRITE_P(node->children[num_keys+1],temp_node_2);
          for(unsigned i=num_keys; i!=index; --i) {
            void* temp_node_3 = STM_SHARED_READ_P(node->children[i-1]);
            STM_SHARED_WRITE_P(node->children[i], temp_node_3);
            intptr_t temp_key_2 = STM_SHARED_READ(node->keys[i-1]);
            STM_SHARED_WRITE(node->keys[i],temp_key_2);
          }
          STM_SHARED_WRITE_P(node->children[index], result.left);
