...
__attribute__((transaction_safe)) void TPCCTables::internalOrderStatus(TM_ARGDECL Customer* customer, OrderStatusOutput* output) {
    ...
    // retrieve from customer: balance, first, middle, last
     //printf("order status: %ld\n",customer->c_id);
    if(local_exec_mode == 1 || local_exec_mode == 3){
      output->c_balance = SLOW_PATH_SHARED_READ_F(customer->c_balance);
    }
    else{
      output->c_balance = FAST_PATH_SHARED_READ_F(customer->c_balance);
    }
...
/*    __transaction_atomic { */
    // FIXME(nmld): transaction block here
		...
		local_exec_mode=0;
		if(local_exec_mode==1){
     	   SLOW_PATH_BEGIN(5,ro);
    	}else{
        	FAST_PATH_BEGIN(5,ro);
    	}
...
//	 Warehouse* w = findWarehouse(TM_ARG warehouse_id);
    if(local_exec_mode == 1 || local_exec_mode == 3){ 
      ...
      SLOW_PATH_SHARED_WRITE_F(d->d_ytd, SLOW_PATH_SHARED_READ_F(d->d_ytd)+h_amount);
		SLOW_PATH_SHARED_WRITE_F(customer->c_balance, SLOW_PATH_SHARED_READ_F(customer->c_balance)-h_amount);
     SLOW_PATH_SHARED_WRITE_F(customer->c_ytd_payment, SLOW_PATH_SHARED_READ_F(customer->c_ytd_payment)+h_amount);
     SLOW_PATH_SHARED_WRITE(customer->c_payment_cnt, SLOW_PATH_SHARED_READ(customer->c_payment_cnt)+1);
    } else{
	...
	FAST_PATH_SHARED_WRITE_F(customer->c_balance, FAST_PATH_SHARED_READ_F(customer->c_balance)-h_amount);
     FAST_PATH_SHARED_WRITE_F(customer->c_ytd_payment, FAST_PATH_SHARED_READ_F(customer->c_ytd_payment)+h_amount);
...
/*    __transaction_atomic { */
    ...
    //TM_THREAD_ENTER();
     int ro = 0;
		local_exec_mode=1;
		if(local_exec_mode==1){
           SLOW_PATH_BEGIN(6,ro);
        }else{
            FAST_PATH_BEGIN(6,ro);
        }
		...
		char date_now[DATETIME_SIZE+1];
            ...
            if(local_exec_mode == 1 || local_exec_mode == 3){
  	          SLOW_PATH_SHARED_WRITE_F(c->c_balance, SLOW_PATH_SHARED_READ_F(c->c_balance)+total);
  	          SLOW_PATH_SHARED_WRITE(c->c_delivery_cnt, SLOW_PATH_SHARED_READ(c->c_delivery_cnt)+1);
            } else{
  	          FAST_PATH_SHARED_WRITE_F(c->c_balance, FAST_PATH_SHARED_READ_F(c->c_balance)+total);
