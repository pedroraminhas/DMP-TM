...
void TPCCGenerator::generateWarehouse(int32_t id, Warehouse* warehouse) {
    ...
    balances[id-1].counter=Warehouse::INITIAL_YTD;
...
void TPCCGenerator::generateDistrict(int32_t id, int32_t w_id, District* district) {
    ...
    district_balances[(w_id-1)*10+(id-1)].counter=District::INITIAL_YTD;
...
void TPCCGenerator::generateCustomer(int32_t id, int32_t d_id, int32_t w_id, bool bad_credit,
    ...
    customer_balances[(w_id-1)*10*3000+(d_id-1)*3000+(id-1)].counter = Customer::INITIAL_BALANCE;
    client_balances[(w_id-1)*10*3000+(d_id-1)*3000+(id-1)].counter=Customer::INITIAL_YTD_PAYMENT;
    //customer->c_ytd_payment = Customer::INITIAL_YTD_PAYMENT;
     client_balances_cnt[w_id*10*4+d_id*4+id].counter=Customer::INITIAL_PAYMENT_CNT;
    //customer->c_payment_cnt = Customer::INITIAL_PAYMENT_CNT;
    //customer->c_delivery_cnt = Customer::INITIAL_DELIVERY_CNT;
    client_delivery_cnt[w_id*10*4+d_id*4+id].counter = Customer::INITIAL_DELIVERY_CNT;
