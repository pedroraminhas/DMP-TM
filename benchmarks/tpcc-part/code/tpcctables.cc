...
__attribute__((transaction_safe)) void TPCCTables::internalOrderStatus(TM_ARGDECL Customer* customer, OrderStatusOutput* output) {
   ...
   long client_offset = (customer->c_w_id-1)*10*3000+(customer->c_d_id-1)*3000+(customer->c_id-1); 
    if(customer->c_id > 2){
    
	if(local_exec_mode == 1 || local_exec_mode == 3){
      output->c_balance = SLOW_PATH_SHARED_READ_F(customer_balances[client_offset].counter);
    }    
    else{
      output->c_balance = FAST_PATH_SHARED_READ_F(customer_balances[client_offset].counter);
    } 
...
/*    __transaction_atomic { */
	...
	bool result;
		local_exec_mode=1;
		if(local_exec_mode==1){
        SLOW_PATH_BEGIN(3,ro);
    }else{
        FAST_PATH_BEGIN(3,ro);
    }
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderRemote(TM_ARGDECL int64_t home_warehouse, int64_t remote_warehouse,
    ...
    for (int i = 0; i < getVectorSize(items); ++i) {
        ...
        if(local_exec_mode == 1 || local_exec_mode == 3){
          if (SLOW_PATH_SHARED_READ(stock->s_quantity) >= item.ol_quantity + 10) {
            SLOW_PATH_SHARED_WRITE(stock->s_quantity, SLOW_PATH_SHARED_READ(stock->s_quantity)-item.ol_quantity);
          } else {
  	        SLOW_PATH_SHARED_WRITE(stock->s_quantity, SLOW_PATH_SHARED_READ(stock->s_quantity)-item.ol_quantity+91);
          }
        	SLOW_PATH_SHARED_WRITE(stock->s_ytd, SLOW_PATH_SHARED_READ(stock->s_ytd)+item.ol_quantity);
        	SLOW_PATH_SHARED_WRITE(stock->s_order_cnt, SLOW_PATH_SHARED_READ(stock->s_order_cnt)+1);
                  // newOrderHome calls newOrderRemote, so this is needed
          if (item.ol_supply_w_id != home_warehouse) {
              // remote order
  	          SLOW_PATH_SHARED_WRITE(stock->s_remote_cnt, SLOW_PATH_SHARED_READ(stock->s_remote_cnt)+1);
          }
        } else{
          if (FAST_PATH_SHARED_READ(stock->s_quantity) >= item.ol_quantity + 10) {
            FAST_PATH_SHARED_WRITE(stock->s_quantity, FAST_PATH_SHARED_READ(stock->s_quantity)-item.ol_quantity);
          } else {
  	         FAST_PATH_SHARED_WRITE(stock->s_quantity, FAST_PATH_SHARED_READ(stock->s_quantity)-item.ol_quantity+91);
          }
        	FAST_PATH_SHARED_WRITE(stock->s_ytd, FAST_PATH_SHARED_READ(stock->s_ytd)+item.ol_quantity);
        	FAST_PATH_SHARED_WRITE(stock->s_order_cnt, FAST_PATH_SHARED_READ(stock->s_order_cnt)+1);
          // newOrderHome calls newOrderRemote, so this is needed
          if (item.ol_supply_w_id != home_warehouse) {
              // remote order
  	          FAST_PATH_SHARED_WRITE(stock->s_remote_cnt, FAST_PATH_SHARED_READ(stock->s_remote_cnt)+1);
          }
        }
    }
...
/*    __transaction_atomic { */
    ...
    District* d  = findDistrict_(warehouse_id, district_id);
     ...
     long client_offset = (warehouse_id-1)*10*3000+(district_id-1)*3000+customer_id-1;
     int district_offset = (warehouse_id-1)*10+(district_id-1);
        int ro = 0;
		local_exec_mode=0;
		if(local_exec_mode==1){
     	   SLOW_PATH_BEGIN(4,ro);
    	}else{
        	FAST_PATH_BEGIN(4,ro);
    	}
...
//	 Warehouse* w = findWarehouse(TM_ARG warehouse_id);
    if(local_exec_mode == 1 || local_exec_mode == 3){ 
      //SLOW_PATH_SHARED_WRITE_F(w->w_ytd, SLOW_PATH_SHARED_READ_F(w->w_ytd)+h_amount);
      SLOW_PATH_SHARED_WRITE_F(balances[warehouse_id].counter, SLOW_PATH_SHARED_READ_F(balances[warehouse_id].counter)+h_amount);

      SLOW_PATH_SHARED_WRITE_F(district_balances[district_offset].counter, SLOW_PATH_SHARED_READ_F(district_balances[district_offset].counter)+h_amount);
		SLOW_PATH_SHARED_WRITE_F(customer_balances[client_offset].counter, SLOW_PATH_SHARED_READ_F(customer_balances[client_offset].counter)-h_amount);
     SLOW_PATH_SHARED_WRITE_F(client_balances[client_offset].counter, SLOW_PATH_SHARED_READ_F(client_balances[client_offset].counter)+h_amount);
     SLOW_PATH_SHARED_WRITE(client_balances_cnt[client_offset].counter, SLOW_PATH_SHARED_READ(client_balances_cnt[client_offset].counter)+1);
    } else{
    FAST_PATH_SHARED_WRITE_F(balances[warehouse_id].counter , FAST_PATH_SHARED_READ_F(balances[warehouse_id].counter)+h_amount);
    FAST_PATH_SHARED_WRITE_F(district_balances[district_offset].counter, FAST_PATH_SHARED_READ_F(district_balances[district_offset].counter)+h_amount);
	FAST_PATH_SHARED_WRITE_F(customer_balances[client_offset].counter, FAST_PATH_SHARED_READ_F(customer_balances[client_offset].counter)-h_amount);
    FAST_PATH_SHARED_WRITE_F(client_balances[client_offset].counter, FAST_PATH_SHARED_READ_F(client_balances[client_offset].counter)+h_amount);
     FAST_PATH_SHARED_WRITE(client_balances_cnt[client_offset].counter, FAST_PATH_SHARED_READ(client_balances_cnt[client_offset].counter)+1);
    } 
...
__attribute__((transaction_safe)) void TPCCTables::paymentHome(TM_ARGDECL int64_t warehouse_id, int64_t district_id, int64_t c_warehouse_id,
    ...
    if(local_exec_mode == 1 || local_exec_mode == 3){
      //SLOW_PATH_SHARED_WRITE_F(w->w_ytd, SLOW_PATH_SHARED_READ_F(w->w_ytd)+h_amount);
      District* d = findDistrict(TM_ARG warehouse_id, district_id);
      //SLOW_PATH_SHARED_WRITE_F(d->d_ytd, SLOW_PATH_SHARED_READ_F(d->d_ytd)+h_amount);
    } else{
      //FAST_PATH_SHARED_WRITE_F(w->w_ytd, FAST_PATH_SHARED_READ_F(w->w_ytd)+h_amount);

      District* d = findDistrict(TM_ARG warehouse_id, district_id);
      //FAST_PATH_SHARED_WRITE_F(d->d_ytd, FAST_PATH_SHARED_READ_F(d->d_ytd)+h_amount);
    }
/*
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(TM_ARGDECL int64_t warehouse_id, int64_t district_id, Customer* c,
    ...
    if(local_exec_mode == 1 || local_exec_mode == 3){
    /* SLOW_PATH_SHARED_WRITE_F(c->c_balance, SLOW_PATH_SHARED_READ_F(c->c_balance)-h_amount);
     SLOW_PATH_SHARED_WRITE_F(c->c_ytd_payment, SLOW_PATH_SHARED_READ_F(c->c_ytd_payment)+h_amount);
     SLOW_PATH_SHARED_WRITE(c->c_payment_cnt, SLOW_PATH_SHARED_READ(c->c_payment_cnt)+1);
*/
      //output->c_credit_lim = SLOW_PATH_SHARED_READ_F(c->c_credit_lim);
      //output->c_discount = c->c_discount;
     // output->c_balance = SLOW_PATH_SHARED_READ_F(c->c_balance);
    } else{
  /*   FAST_PATH_SHARED_WRITE_F(c->c_balance, FAST_PATH_SHARED_READ_F(c->c_balance)-h_amount);
     FAST_PATH_SHARED_WRITE_F(c->c_ytd_payment, FAST_PATH_SHARED_READ_F(c->c_ytd_payment)+h_amount);
     FAST_PATH_SHARED_WRITE(c->c_payment_cnt, FAST_PATH_SHARED_READ(c->c_payment_cnt)+1);
*/
      //output->c_credit_lim = FAST_PATH_SHARED_READ_F(c->c_credit_lim);
     // output->c_discount = c->c_discount;
     // output->c_balance = FAST_PATH_SHARED_READ_F(c->c_balance);
    }
}
    ...
    //TM_THREAD_ENTER();
     int ro = 0;
		...
		char date_now[DATETIME_SIZE+1];
            ...
            long client_offset = (warehouse_id-1)*10*3000+(d_id-1)*3000+(o->o_c_id-1);
            
            if(local_exec_mode == 1 || local_exec_mode == 3){
  	          SLOW_PATH_SHARED_WRITE_F(customer_balances[client_offset].counter, SLOW_PATH_SHARED_READ_F(customer_balances[client_offset].counter)+total);
                //SLOW_PATH_SHARED_WRITE_F(client_balances[client_offset].counter, SLOW_PATH_SHARED_READ_F(client_balances[client_offset].counter)+total);
  	          //SLOW_PATH_SHARED_WRITE(c->c_delivery_cnt, SLOW_PATH_SHARED_READ(c->c_delivery_cnt)+1);
              //SLOW_PATH_SHARED_WRITE(client_delivery_cnt[client_offset].counter, SLOW_PATH_SHARED_READ(client_delivery_cnt[client_offset].counter)+1);
            } else{
              FAST_PATH_SHARED_WRITE_F(customer_balances[client_offset].counter, FAST_PATH_SHARED_READ_F(customer_balances[client_offset].counter)+total);
  	          //FAST_PATH_SHARED_WRITE_F(client_balances[client_offset].counter, FAST_PATH_SHARED_READ_F(client_balances[client_offset].counter)+total);
  	          //FAST_PATH_SHARED_WRITE(c->c_delivery_cnt, FAST_PATH_SHARED_READ(c->c_delivery_cnt)+1);
              FAST_PATH_SHARED_WRITE(client_delivery_cnt[client_offset].counter, FAST_PATH_SHARED_READ(client_delivery_cnt[client_offset].counter)+1);
            }
    }
	if(local_exec_mode==1){
           SLOW_PATH_END();
