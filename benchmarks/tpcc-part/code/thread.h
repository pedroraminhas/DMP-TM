...
extern __attribute__((aligned(CACHE_LINE_SIZE))) padded_scalar_t sched_mode[];
extern __thread __attribute__((aligned(CACHE_LINE_SIZE))) padded_scalar_t sched_capacity[];
extern __thread __attribute__((aligned(CACHE_LINE_SIZE))) padded_scalar_t sched_htm_commits[];
...
extern volatile long global_transition_count;
extern volatile long writer_count[80][20000000];

//Metadata structure
typedef struct{
        volatile long status_field;       //protection of pge
        volatile long writer_count;       //how many STM writers
        volatile long transition_count;   //how many times HTM reverted to normal R/W access
        volatile long lock_bit;           //Indicates if a system call is being issued
}metadata;
...
extern long pages_read_heap[80][20000000];
extern long pages_written_heap[80][20000000];
...
extern  __thread long local_transition_count;
extern __thread long* transition_count_pages_read;

extern int number_threads;
extern int sched_threshold;
extern volatile int htm_retries;
...
# define CHANGE_PROTECTION_HTM_READ_WRITE(pageID) ({ \
    ...
    long increment_page_loop=0; \
    long result; \
    BEGIN_HANDLER_WRITE: for (increment_inside_loop=0;increment_inside_loop<number_threads;increment_inside_loop++){ \
                while(writer_count[increment_inside_loop][pageID]){ cpu_relax();} \
       } \
        while(1){ \
                        while(LOCKED(metadata_array[pageID].lock_bit)){ \
                            __asm__ ("nop;"); \
    ...
    if(result ==0){ \
        ...
        metadata_array[pageID].status_field = (PROT_NONE); \
        metadata_array[pageID].lock_bit=0;  \
        goto BEGIN_HANDLER_WRITE;  \
    }   \
})
