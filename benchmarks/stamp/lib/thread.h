...
extern __thread __attribute__((aligned(CACHE_LINE_SIZE))) padded_scalar_t capacity[];
extern __thread __attribute__((aligned(CACHE_LINE_SIZE))) padded_scalar_t htm_commits[];
...
# define CHANGE_PROTECTION_HTM_READ_WRITE(pageID) ({ \
    ...
    long increment_page_loop=0; \
    long result; \
    BEGIN_HANDLER: for (increment_inside_loop=0;increment_inside_loop<number_threads;increment_inside_loop++){ \
