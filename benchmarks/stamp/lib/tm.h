                        ...
                        if (status == _HTM_TBEGIN_STARTED) { \
                            ...
                            break; \
                            	...
                            	if (__TM_is_footprint_exceeded(&TM_buff)) { \
									...
									long temp_cap = capacity[num_tx].counter++; \
                                    long temp = htm_commits[num_tx].counter+temp_cap; \
                                    if(temp>50 && ((temp_cap * 100/ temp) >= THRESHOLD_CAPACITY)){ \
                                        mode2[num_tx].counter=1; \
                                        STM_BEGIN(b,ro); \
                                        break; \
                                    } \
                                    /*} else{ \
                                        if(mode2[num_tx].counter==1){\
                                                STM_BEGIN(b,ro); \
                                                break; \
                                        } \
                                    } */\
